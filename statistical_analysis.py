...
# def bootstrap_sparse_tensor(args, df, bootstrap_row_idx, bootstrap_col_idx):
#     '''
#     vectorized implementation of bootstrapping a sparse tensor. this is SLOWER than the non-vectorized implementation below
...
#     RETURNS the result_str, means, and the z_idx used, which depend on the row_idx and col_idx used at each step
#     '''
#     means = []
#     bootstrap_z_idx = []
#     # step one, make the tensor    
#     tensor = np.full((df.shape[0], df.shape[1], df.shape[0]), np.nan) # assuming, z dimension is same length as first dimension
...
#                     tensor[row_num, col_num, data_id] = stat
#     # now bootstrap the tensor
#     print("bootstrapping tensor")
#     # import pdb; pdb.set_trace()
#     for i in range(args.num_samples):
#         print(i)
#         row_idx = bootstrap_row_idx[i]
#         col_idx = bootstrap_col_idx[i]
#         # where_z_all_missing = np.argwhere(np.isnan(tensor[row_idx,col_idx,:]).sum((0,1)) == (tensor.shape[0] * tensor.shape[1])).reshape(-1)
#         # eligible_z_idx = np.setdiff1d(tensor.shape[-1]), where_z_all_missing)
#         z_idx = np.random.choice(np.arange(tensor.shape[-1]), size=tensor.shape[-1], replace=True)        
#         # x_sample = tensor[row_idx, col_idx, z_idx]
#         x_sample = tensor[row_idx, :, :] 
#         x_sample = x_sample[:, col_idx, :] 
#         x_sample = x_sample[:, :, z_idx] 
...
if __name__ == "__main__":
    ...
    # metrics = ['main_acc', 
    #             'upd_suc', # main update success
    #             'par_eq_mean', 'before_par_eq_mean', # paraphrase update success
    #             'cons',
    #             'dep_acc', 'before_dep_acc', 'dep_chg', # entailed update success
    #             'after_ctp_acc', 'before_ctp_acc', 'ctp_chg', # contrapositive update success
    #             'oth_acc',     'before_acc',     'acc_chg', # change in acc for other data
    #             'oth_odp_acc', 'before_odp_acc', 'odp_chg', # change in acc for other entailed data (entailed by other points)
    ...
    # metrics.append('oth_ret')
    # metrics = [
    #     # 'par_eq_mean',  #'after_ret',
    #     # 'after_acc',     'before_acc',     'acc_chg',
    #     'dep_acc',
    #     'after_odp_acc', 'combined_acc_chg', # change in acc for other data
    #     # 'ind_ret', # retain prediction rate for neutral and other data
    #     'after_ret', 'after_odp_ret', 'combined_ret_chg'
    # ]
    # metrics = ['oth_ret', 'after_ret']
    ...
    # hypothesis testing
        ...
        for hypothesis_test in hypothesis_tests:
            ...
            if not set(A_keys) == set(B_keys): print("metrics not the same between both experiments")
            for key in A_keys:
